...
from matcher import MatchConfig, VisionMatcher, list_images

APP_NAME = "SORT WEAR GEAR v0.4 MULTI-MATCH + REVIEW"
MODEL = None
...
def safe_copy(src_path, target_dir):
    ...
    if out.exists() and out.resolve() != src.resolve():
        # identical target name means this reference already exists; never overwrite it.
        i = 1
        while out.exists():
...
def copy_many(items, target_dir):
    ...
    for item in items or []:
        p = file_path(item)
        if p and Path(p).is_file(): copied.append(safe_copy(p, target_dir))
    return copied
...
def load_existing(destination):
    ...
    paths = ensure_library(root)
    return (
        f"LIBRARY OPENED\nPEOPLE {len(list_images(paths['people']))} · GEAR {len(list_images(paths['gear']))} · ASSEMBLAGES {len(list_images(paths['to_sort']))}",
        str(root),
    )
...
def build_indexes(root: Path, predictions: List[dict]):
    paths = ensure_library(root)
    # SORTED and REVIEW only contain generated copies. Rebuild them from source every run.
    ...
    for pred in predictions:
        ...
        garments = [g["name"] if isinstance(g, dict) else g for g in pred.get("garments", [])]
...
def sort_library(people_files, gear_files, assembly_files, destination, person_threshold, garment_threshold, max_garments):
    ...
    people = list_images(paths["people"])
    gear = list_images(paths["gear"])
    assemblies = list_images(paths["to_sort"])
    ...
    for img in assemblies:
        ...
        if saved:
            ...
            pred["garments"] = [{"name": g, "score": 1.0, "region": "human", "margin": 1.0} for g in saved.get("garments", [])]
    ...
    manifest = {
        ...
        "workflow": ["SCAN","MATCH PEOPLE","MATCH GEAR","MULTI-GARMENT MATCH","REVIEW AMBIGUOUS","CORRECT / VALIDATE","BUILD LIBRARY"],
    }
    save_json(root / "library.json", manifest)
    ambiguous = [p["image"] for p in predictions if p.get("ambiguous")]
    status = (
        f"DONE · {len(assemblies)} ASSEMBLAGES\n"
        f"PEOPLE {len(people)} · GEAR {len(gear)}\n"
        f"AMBIGUOUS {len(ambiguous)} → REVIEW\n"
        f"REPORT → {root / 'sorting_report.csv'}\n"
        f"LIBRARY → {root}"
    )
...
def current_refs(root_s: str):
    ...
    paths = ensure_library(root)
    return [p.stem for p in list_images(paths["people"])], [p.stem for p in list_images(paths["gear"])]
...
def load_review_item(image_name, root_s):
    ...
    garments = [g["name"] if isinstance(g, dict) else g for g in pred.get("garments", [])]
    detail = json.dumps({
        "person_top": pred.get("person_top", []),
        "garment_alternatives": pred.get("garment_alternatives", {}),
    }, indent=2, ensure_ascii=False)
    return (
        pred.get("image_path"),
        gr.Dropdown(choices=people, value=pred.get("person")),
        gr.CheckboxGroup(choices=gear, value=garments),
        detail,
    )
...
def save_review(image_name, person, garments, root_s):
    ...
    root = Path(root_s)
    corrections = load_json(root / "corrections.json", {})
    corrections[image_name] = {"person": person, "garments": list(garments or [])}
    ...
    for p in predictions:
        if p.get("image") == image_name:
            p["person"] = person
            p["garments"] = [{"name": g, "score": 1.0, "region": "human", "margin": 1.0} for g in (garments or [])]
            p["ambiguous"] = False
    ...
    return f"VALIDATED · {image_name} → {person} + {', '.join(garments or [])}", gr.Dropdown(choices=remaining, value=(remaining[0] if remaining else None))


with gr.Blocks(title=APP_NAME, css=CSS) as demo:
    gr.HTML(LOGO)
    gr.Markdown("# SORT · WEAR · GEAR — v0.4")
    gr.Markdown("**Hybrid local matching: OpenCLIP semantic + DINOv2 instance features + body-region multi-match.**")
    ...
    with gr.Tab("SORT"):
        ...
        with gr.Row():
            ...
            run = gr.Button("SCAN → MULTI-MATCH → BUILD", elem_classes=["primary"])
    ...
    run.click(
        sort_library,
        inputs=[people, gear, assemblies, destination, person_threshold, garment_threshold, max_garments],
        outputs=[status, review_select, root_state],
    )
